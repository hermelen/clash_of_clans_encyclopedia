...
class DarkSpellFactoryController extends Controller
{
  public function indexAction()
  {
    ...
    if (null === $singles) {
      throw new NotFoundHttpException("Aucun Usine de Sorts Noirs n'est répertorié.");
    } else {
  ...
  public function addAction(Request $request, Batiment $batiment)
  {
    ...
    if ($request->isMethod('POST') && $form->handleRequest($request)->isValid()) {
      ...
      $request->getSession()->getFlashBag()->add('notice', 'Usine de Sorts Noirs bien enregistré.');

      return $this->redirectToRoute('her_dark_spell_factory_single', array('level' => $single->getLevel()));
    }
  ...
  {
    $em = $this->getDoctrine()->getManager();
    $batiment = $em->getRepository('HerItemsBundle:Batiment')->findOneBy(array( 'id' => 7 ));
    $single = $em->getRepository('HerItemsBundle:SpellFactory')->findOneBy(array(
      'batiment'=>$batiment,
      'level'=>$level
    ));
    ...
    if (null === $single) {
      throw new NotFoundHttpException("Le Usine de Sorts Noirs ".$level." n'est pas répertorié");
    }
