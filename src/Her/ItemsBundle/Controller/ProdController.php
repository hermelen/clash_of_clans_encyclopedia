...
class ProdController extends Controller
{
  ...
  public function indexAction(Batiment $general)
  {
    ...
    $singles = $em->getRepository('HerItemsBundle:Prod')->orderByLevelFromBatiment($general);

    if (null === $general) {
      throw new NotFoundHttpException("Aucun '.$general->getName().' n'est répertorié.");
    } else {
      return $this->render('HerItemsBundle:Prod:index.html.twig',array(
        'singles'               => $singles,
        'general'              => $general,
   ...
   */
  public function singleAction(Batiment $general, Prod $level)
  {
    $repo = $this
    ->getDoctrine()
    ->getManager()
    ->getRepository('HerItemsBundle:Prod');
    ...
    if (null === $single) {
      ...
      $fullDuration = $single->getFull();

      $formattedImprovementDuration = $formatDuration->durationSec($improvementDuration);
      $formattedFullDuration = $formatDuration->durationSec($fullDuration);

      return $this->render('HerItemsBundle:Prod:single.html.twig',array(
        'general'                     => $general,
        'single'                      => $single,
        'best'                        => $best,
        'formattedImprovementDuration'=> $formattedImprovementDuration,
        'formattedFullDuration'       => $formattedFullDuration,
      ));
    }
  }
  ...
  public function addAction(Request $request, Batiment $general)
  {
    ...
    $form = $this->get('form.factory')->create(ProdType::class, $single);
    ...
    if ($request->isMethod('POST') && $form->handleRequest($request)->isValid()) {
      ...
      return $this->redirectToRoute('her_prod_single', array(
        'slug' => $general->getSlug(),
        'level'    => $single->getLevel()
      ));
    }

    return $this->render('HerItemsBundle:Prod:add.html.twig', array(
   ...
   * @ParamConverter("level", options={"mapping": {"level": "level"}})
   */
  public function editAction(Batiment $general, Prod $level, Request $request)
  {
    $em = $this->getDoctrine()->getManager();

    $single = $em->getRepository('HerItemsBundle:Prod')->findOneBy(array(
      'level'   => $level->getLevel(),
      'batiment'=> $general
      ));
    ...
    return $this->render('HerItemsBundle:Prod:edit.html.twig', array(
      'single'     => $single,
      'form'     => $form->createView(),
    ));
  }
   ...
   * @ParamConverter("level", options={"mapping": {"level": "level"}})
   */
  public function deleteAction(Batiment $general, Prod $level, Request $request)
  {
    $em = $this->getDoctrine()->getManager();
    ...
    if ($request->isMethod('POST') && $form->handleRequest($request)->isValid()) {
      ...
      return $this->redirectToRoute('her_prod_homepage', array(
        'slug' => $general->getSlug(),
      ));
    }

    return $this->render('HerItemsBundle:Prod:delete.html.twig', array(
      'single'     => $single,
      'form'     => $form->createView(),
    ));
  }
